...
export default class Store {
    ...
    public hydrateWithEntities = (entities: Map<string, Entity>) => {
        assert(this.state.size === 0, "State is not empty. Please use state.clear() to clear the state before hydrating.");
        this.state = entities;
    }
    ...
    public assertStateSnapshotEq = (snapshot: string) => {
        assert(this.state.size > 0, "Cannot check for equality when the state is empty. You need to first hydrate the state.");
    ...
    public assertEntityEq = (entityKey: string, entity: Entity) => {
        ...
        assert(this.state.get(entityKey) !== undefined, `Entity key ${entityKey} not found in the state.`);
